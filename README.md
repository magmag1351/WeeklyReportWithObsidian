...
## 今後の予定（将来の予定）の管理 (`future.py`)

本日行うことではないが、将来行う必要ができたタスク（今後の予定）を登録・管理し、自動的または手動で当日のデイリーノートに転記できる機能です。

### 主な機能
- **今後の予定の一括管理**: `config.json` に設定した `future_tasks.md`（デフォルト）に、将来の予定を Dataview 形式 `[scheduled:: YYYY-MM-DD]` でプールしておきます。
- **自動転記**: `today.py` を実行した際、予定日が実行日（または指定日）になったタスク、あるいは予定日を過ぎた未完了タスクが、本日のデイリーノートの `# 本日の予定` セクションに自動的に転記されます。転記されたタスクは `future_tasks.md` 側で完了（`- [x]`）に変更され、転記済みを示す `[copied:: YYYY-MM-DD]` が付与されます。
- **一覧からの即時転記 (`-l` オプション)**: 今日の予定に日付未指定（Someday）のタスクや前倒しで行いたい予定を、一覧から手動で選択して本日のノートに転記できます。

### 使い方

#### 1. 対話モードで予定を追加する
引数なしで実行すると、タスク名、予定日、カテゴリ、優先度を対話形式で入力できます。
```bash
python future.py
```

#### 2. 引数付きで予定を追加する
ターミナルから直接オプションを指定して予定を追加できます。
```bash
python future.py --task "COMSOLのシミュレーション開始" --date 2026-07-20 --category 研 --priority B
```
* `--task` (または `-t`): タスク内容（必須）
* `--date` (または `-d`): 予定日（`YYYY-MM-DD` または `YYYYMMDD`。任意。指定しない場合は日付未指定として登録されます）
* `--category` (または `-c`): カテゴリ（`研` または `私`。任意）
* `--priority` (または `-p`): 優先度（`A`, `B`, `C` など。任意）

#### 3. 予定を一覧表示して現時点で本日のデイリーノートに転記する
`-l` (または `--list`) オプションを付けて実行すると、登録されている未完了の予定一覧が表示され、選択した予定を本日のノートへ即座に転記できます。
```bash
python future.py -l
```
```text
--- 今後の予定一覧 ---
[1] COMSOLのシミュレーション開始 [scheduled:: 2026-07-20] [category:: 研] [priority:: B]
[2] 買い物 [category:: 私]
----------------------

転記する予定の番号を指定してください（カンマ区切りで複数指定可、例: 1, 3。キャンセルは 'q'）: 2
```
